...
class SnakemakeRuleWriter(object):
    ...
    def write_dynamic_rules(self, rule_name, input_json, wildcard_inputs, shell_template, parameters, wildcard_outputs, output_json):
        ...
        dynamic_output_kvs = ', '.join("%s=dynamic('%s')"%(k, os.path.normpath(v)) for k,v in wildcard_inputs.iteritems())
        dynamic_input_kvs =  ', '.join("%s=ancient(dynamic('%s'))"%(k, os.path.normpath(v)) for k,v in wildcard_outputs.iteritems())
...
rule dynamic_%(rule_name)s_split:
    ...
    shell: 'python -m falcon_unzip.mains.symlink_mapped --special-split={input} %(wi_pattern_kvs)s'
"""%(locals())
        self.write(rule)
